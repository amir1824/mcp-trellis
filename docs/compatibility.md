...
Updated 2026-09-12. Repository implementation and automated coverage are in good
...
|---|---|---|---|
| Claude custom connector | `claude`: DCR, public client, S256 PKCE; built-in callback | `test/oauth/clients.test.ts`, `dcr.test.ts`, `pkce.test.ts`; saas-demo e2e + `scripts/prep-claude-oauth.mjs` | Pending (UI) |
...
`src/protocol.ts` implements `2024-11-05`, `2025-03-26`, `2025-06-18`. Initialization defaults to `2025-06-18`; absent request headers use `2025-03-26`. This is an implementation inventory, not a claim to implement the latest revision. CIMD and newer protocol revisions remain separate hardening work.
