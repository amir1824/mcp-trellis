...
import { VALID_S256_CHALLENGE } from "../../helpers/oauth.js";
...
describe("consent ticket lifecycle", () => {
  ...
  it("does not burn the rightful user's ticket when a different user posts it first", async () => {
    let currentUser = "alice";
    const router = createOAuthRouter({
      allowUnregisteredClients: true,
    ...
    const ticket = await getTicket(router);
    currentUser = "bob";
    const wrong = await postConsent(router, { consent_ticket: ticket, approved: "true" });
    assert.ok(wrong);
    await expectOAuthError(wrong, 400, "invalid_grant");

    currentUser = "alice";
    const right = await postConsent(router, { consent_ticket: ticket, approved: "true" });
    assert.ok(right);
    assert.equal(right.status, 302);
    assert.ok(new URL(right.headers.get("Location") ?? "").searchParams.get("code"));
  });
