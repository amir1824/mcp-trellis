...
describe("shared codeStore single-use across two handlers", () => {
  ...
  it("keeps single-use across routers built per request in in-memory mode", async () => {
    // `buildApp(env).fetch(req)` builds a fresh router for every request; the
    // in-memory store must be process-wide, not per router, or replay works.
    const build = () =>
      createOAuthRouter({
        allowUnregisteredClients: true,
        requireRegisteredClients: false,
    ...
    const { code, challenge: verifier } = await issueCode(build());
    const first = await redeem(build(), code, verifier);
    assert.equal(first?.status, 200);

    const second = await redeem(build(), code, verifier);
    assert.equal(second?.status, 400);
    const body = (await second?.json()) as { error: string };
    assert.equal(body.error, "invalid_grant");
  });
