...
/**
 * Structural subset of `Request`/`Response` that `readBoundedBytes` actually
...
export type BoundedBodySource = {
  ...
  text: () => Promise<string>;
  /** Preferred over `text` for the no-stream fallback — keeps binary bodies intact. */
  arrayBuffer?: () => Promise<ArrayBuffer>;
};

/**
 * Reads a bounded body as raw bytes, rejecting early on an honest oversized
 ...
 * Falls back to `source.arrayBuffer()` (or `source.text()`) when
 * `source.body` is null (some runtimes don't expose a streaming body),
 * still enforcing the cap.
 *
 * Optional `signal` cancels the active reader on abort — needed when a hop
 * timeout must stop a stalled stream (a locked `body.cancel()` is a no-op).
 */
export const readBoundedBytes = async (
  ...
  if (!reader) {
    // ponytail: no stream → arrayBuffer/text ignore signal; fine for request
    // bodies. CIMD always has a stream on Node/undici. Upgrade: race abort
    // or refuse the fallback when signal is required.
    const bytes = source.arrayBuffer
      ? new Uint8Array(await source.arrayBuffer())
      : new TextEncoder().encode(await source.text());
  ...
  try {
    for (;;) {
      ...
      // Cancel may settle read() as done — still treat abort as failure.
      if (signal?.aborted) throw new Error("body read aborted");
      if (done) break;
...
/** `readBoundedBytes`, decoded as UTF-8 — for JSON / form / text bodies. */
export const readBoundedText = async (
  source: BoundedBodySource,
  maxBytes: number,
  signal?: AbortSignal,
): Promise<string> => new TextDecoder().decode(await readBoundedBytes(source, maxBytes, signal));
